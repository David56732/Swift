...
class TableViewController: UITableViewController{
    ...
    @IBAction func pushAddAction(_ sender: UIBarButtonItem) {
       let alertController = UIAlertController(title: "New item", message: nil, preferredStyle: .alert)
        ...
        alertController.addTextField { (textFeild) in
            textFeild.placeholder = "New item name"
        }
        ...
        let alertAction2 = UIAlertAction(title: "Cancel", style: .destructive, handler: nil)
    ...
    override func viewDidLoad() {
        ...
        let badgeNumber : Int = countNotCompleted()
        let application = UIApplication.shared
        let center = UNUserNotificationCenter.current()
        center.requestAuthorization(options: [.badge, .alert, .sound]){ (success, error)
            in
            print ("Success")}
